...
export class AppService {
  // protected jobLocationList: JobLocation[] = [
  //   {
  //     "id": 1,
  //     "company": "Scoot",
  ...
  // getJobLocationById(id: Number): JobLocation | undefined {
  //   return this.jobLocationList.find(jobLocation => jobLocation.id === id)
  // }
  
  public getJobById(id: number): Observable<JobLocation | undefined> {
    return this.getJobLocationById(id: Number).pipe(
      map((jobLocation: any) => jobLocation.find((jobLocation: any) => jobLocation.id === id))
    );
  }
  getJobLocationById(id: number, Number: NumberConstructor) {
